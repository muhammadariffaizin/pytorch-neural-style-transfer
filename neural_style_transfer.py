...
def build_loss(neural_net, optimizing_img, mrf_layers, loss_layers, loss_fns, targets, weights, style_layers, style_patches_lists, weight_list, patch_size, dist_template, cont_dist, mrf_loss_fn, config, device):
    ...
    total_loss = content_loss + style_loss + mrf_loss + dist_loss + 0.001*l2_reg
...
def neural_style_transfer(config):
    ...
    if config['optimizer'] == 'adam':
        ...
        for cnt in range(num_of_iterations[config['optimizer']]):
            total_loss, content_loss, style_loss, mrf_loss, dist_loss = build_loss(neural_net, optimizing_img, mrf_layers, loss_layers, loss_fns, targets, weights, style_layers, style_patches_lists, weight_list, config['patch_size'], dist_template, cont_dist, mrf_loss_fn, config, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mrf_weight", type=float, help="weight factor for mrf loss", default=1e2)
    parser.add_argument("--dist_weight", type=float, help="weight factor for distance loss", default=1e3)
